...
export function RentalsClientPage({ initialVehicles }: { initialVehicles: Vehicle[] }) {
    ...
    useEffect(() => {
        // Simulate loading effect, but data is already present
                                ...
                                <Card key={vehicle.id} className="flex flex-col overflow-hidden shadow-md transition-all hover:shadow-xl">
                                    <CardHeader className="p-0 relative">
                                        <Link href={`/rentals/${vehicle.id}`}>
                                            <Image src={vehicle.imageUrls?.[0] || 'https://placehold.co/600x400.png'} alt={`${vehicle.make} ${vehicle.model}`} width={600} height={400} className="w-full object-cover aspect-video" data-ai-hint="car front view"/>
                                        </Link>
